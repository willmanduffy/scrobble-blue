...
describe('BlueSky', () => {
  ...
  describe('createAgent', () => {
    ...
    it('should throw if login fails', async () => {
      ...
      await expect(BlueSky.createAgent(mockEnv)).rejects.toThrow('Login failed');
    });
  });
  ...
  describe('updateDescription', () => {
    ...
    it('should throw if update fails', async () => {
      ...
      vi.mocked(agent.upsertProfile).mockRejectedValueOnce(new Error('Update failed'));

      const bluesky = await BlueSky.createAgent(mockEnv);
      await expect(bluesky.updateDescription('test')).rejects.toThrow('Update failed');
    });
  });
}); 